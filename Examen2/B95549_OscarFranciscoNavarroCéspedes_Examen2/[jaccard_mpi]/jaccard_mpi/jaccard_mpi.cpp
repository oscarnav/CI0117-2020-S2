...
int main(int argc, char *argv[])
{
    ...
    double *recv_buffer;
    recv_buffer = new double[num_processes];
    ...
    double max_jaccard_processes = -1.0;
    ...
    while (getline(matrix_file, matrix_row) && getline(text_file, text_row)) {
        ...
        if(my_id == contador){
           //cout << "proceso que le toca analizar: " << contador << " Yo soy: " << my_id << endl;
        ...
        ++contador;  
        if(contador == num_processes){
            contador = 0;
        }
          
    }
    
    MPI_Allgather(&max_jaccard_similarity, 1, MPI_DOUBLE, recv_buffer, 1, MPI_DOUBLE,MPI_COMM_WORLD);


    for(i = 0;i < num_processes; ++i){
        if(recv_buffer[i]>max_jaccard_processes){
            max_jaccard_processes = recv_buffer[i];
        }
    ...
    if(max_jaccard_processes == max_jaccard_similarity){
        cout << "\nEvaluated text: \n\"" << test_text << "\"" << endl;
